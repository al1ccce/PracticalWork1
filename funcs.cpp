...
TableNode* ParseJson(const string& config, configjson& schema){
    ...
    for (int i = inext; i < config.size(); i++) {
        if (config[i] == '"') {
            ...
            string locker = temptablename + "_lock.txt";
            ofstream filelocker(locker);
            filelocker << "unlocked";
            filelocker.close();
...
bool isLocked(const string tableName, configjson schema){
    string filePath = "/home/andrey/Documents/coding/project1/" + schema.name + "/" + tableName;
    ifstream locker(filePath);
    string line;
    if (locker.is_open()) {
        getline(locker, line);
    }
...
// Изменяет состояние блокировки
void lockerChanger(const string tableName, configjson schema){
    string filePath = "/home/andrey/Documents/coding/project1/" + schema.name + "/" + tableName;
    ofstream file(filePath);
    if (isLocked(tableName, schema)){
        file << "unlocked";
    }
    if (!isLocked(tableName, schema)){
        file << "locked";
...
void Insert(const string& command, TableNode* tables, const configjson &schema) {
    ...
    while (iss >> word) { 
        ...
        if (count == 5){
            if (isLocked(temptablename, schema)) {
                cout << "Таблица уже используется\n";
                return;
            }
            lockerChanger(temptablename, schema);
            string pkPath = "/home/andrey/Documents/coding/project1/" + schema.name + "/" + temptablename + "/" + temptablename + "_pk_sequence.txt";
            int fnum = 1;
            ...
            while (true) {
                string filePath = "/home/andrey/Documents/coding/project1/" + schema.name + "/" + temptablename + "/" + to_string(fnum) + ".csv";
            ...
            if (teststr.size() <= 1){
                string cols = copyFirstRow("/home/andrey/Documents/coding/project1/" + schema.name + "/" + temptablename + "/1.csv");
                file << cols << endl;
                test.close();
            }
            file << values;
    ...
    while (iss >> word) { 
        ...
        if (count == 5){
            ...
            if (!columnExists(tables, tempcolname, temptablename)){
                cout << "No column named '" << tempcolname << "'" << endl;
                break;
            }
        }
        if (word != "=" && count == 6){
        ...
        if (count == 7){
            ...
            lockerChanger(temptablename, schema);
            const string directoryPath = "/home/andrey/Documents/coding/project1/" + schema.name + "/" + temptablename;
...
// Печать SELECT FROM
void Selectselect(Sel* SelHead, const configjson &schema){
    string directoryPath1 = "/home/andrey/Documents/coding/project1/" + schema.name + "/" + SelHead->tab;
    string directoryPath2 = "/home/andrey/Documents/coding/project1/" + schema.name + "/" + SelHead->next->tab;
    int csvnum1 = countCSVFiles(directoryPath1);
    int csvnum2 = countCSVFiles(directoryPath2);
        for (size_t fnum = 1; fnum <= csvnum1; fnum++) {
            string filePath = "/home/andrey/Documents/coding/project1/" + schema.name + "/" + SelHead->tab + "/" + to_string(fnum) + ".csv";
                ...
                if (Contains(SelHead->id, pk_value)) {
                    ...
                    if (nextSel != nullptr) {
                        for (size_t fnum2 = 1; fnum2 <= csvnum2; fnum2++) {
                            string filePath2 = "/home/andrey/Documents/coding/project1/" + schema.name + "/" + nextSel->tab + "/" + to_string(fnum2) + ".csv";
...
Sel* Strequals(Sel* SelHead, string tabl1, string tabl2, string col1, string col2, string op, configjson& schema){
    string directoryPath1 = "/home/andrey/Documents/coding/project1/" + schema.name + "/" + tabl1;
    string directoryPath2 = "/home/andrey/Documents/coding/project1/" + schema.name + "/" + tabl2;
    int csvnum1 = countCSVFiles(directoryPath1);
    int csvnum2 = countCSVFiles(directoryPath2);
    int N1, N2 = 15;
    int *arr1 = new int[N1];
    ...
    int arrsize2 = 0;
    for (size_t fnum = 1; fnum <= csvnum1; fnum++) {
        string filePath = "/home/andrey/Documents/coding/project1/" + schema.name + "/" + tabl1 + "/" + to_string(fnum) + ".csv";
        rapidcsv::Document doc1(filePath);
        ...
        for (size_t i = 0; i < doc1.GetRowCount(); ++i) {
            ...
            //cout << pk_value << endl;
                for (size_t fnum2 = 1; fnum2 <= csvnum2; fnum2++) {
                    string filePath2 = "/home/andrey/Documents/coding/project1/" + schema.name + "/" + tabl2 + "/" + to_string(fnum2) + ".csv";
                    rapidcsv::Document doc2(filePath2);
...
void Select(string& command, TableNode* tables, configjson &schema) {
    ...
    while (iss >> word) {
        ...
        const string directoryPath = "/home/andrey/Documents/coding/project1/" + schema.name + "/" + tabword;
        int csvnum = countCSVFiles(directoryPath);
    ...
    currentNode = selectedColumnsHead;
    while (iss2 >> word) {
        word = DelComa(word);
        ...
        if (tempNode == nullptr) {
            cout << "Таблицы не совпадают" << endl;
            return;
        }
    }
    getline(cin, command);
    ...
    iss3 >> word;
    if (word != "WHERE"){
        Selectselect(selectedColumnsHead, schema);
        return;
    }
    else{
        string cond;
